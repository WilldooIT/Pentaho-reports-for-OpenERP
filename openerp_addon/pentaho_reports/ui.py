...
class report_xml(orm.Model):
    ...
    _columns = {
        ...
        'linked_menu_id': fields.many2one(
            'ir.ui.menu', 'Linked menu item', select=True),
        'created_menu_id': fields.many2one('ir.ui.menu', 'Created menu item'),
        # This is not displayed on the client - it is a trigger to
        # indicate that a prpt file needs to be loaded - normally it is
        # loaded by the client interface. In this case, the filename should
        # be specified with a module path.
    ...
    def create_menu(self, cr, uid, vals, context=None):
        view_ids = self.pool.get('ir.ui.view').search(
            cr, uid, [('model', '=', 'ir.actions.report.promptwizard'),
                ('type', '=', 'form')], context=context)
        ...
        action_id = self.pool.get('ir.actions.act_window').create(
            cr, uid, action_vals, context=context)

        result = self.pool.get('ir.ui.menu').create(
            cr, uid, {
                'name': vals.get('name', 'Pentaho Report'),
                'sequence': 10,
    ...
    def delete_menu(self, cr, uid, menu_id, context=None):
        action = self.pool.get('ir.ui.menu').browse(
            cr, uid, menu_id, context=context).action
        if action and action._model._name == 'ir.actions.act_window':
            self.pool.get('ir.actions.act_window').unlink(
                cr, uid, [action.id], context=context)
        result = self.pool.get('ir.ui.menu').unlink(
            cr, uid, [menu_id], context=context)
        return result
    ...
    def update_menu(self, cr, uid, action_report, context=None):
        ...
        if action_report.created_menu_id and not action_report.linked_menu_id:
            self.delete_menu(
                cr, uid, action_report.created_menu_id.id, context=context)
        ...
        if action_report.linked_menu_id:
            ...
            if not action_report.created_menu_id:
                ...
                if action and action._model._name == 'ir.actions.act_window':
                    existing_context = safe_eval(
                        self.pool.get('ir.actions.act_window').browse(
                            cr, uid, action.id, context=context).context)
                    new_context = existing_context if type(existing_context) == dict else {}
                    new_context['service_name'] = action_report.report_name or ''
                    self.pool.get('ir.actions.act_window').write(
                        cr, uid, [action.id], {
                            'name': action_report.name or 'Pentaho Report',
                            'context': str(new_context),
                        }, context=context)

                self.pool.get('ir.ui.menu').write(
                    cr, uid, [action_report.created_menu_id.id], {
                        'name': action_report.name or 'Pentaho Report',
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if context.get("is_pentaho_report", False):
            vals["model"] = self.pool.get("ir.model").browse(
                cr, uid, vals["pentaho_report_model_id"],
        ...
        if vals.get('linked_menu_id', False):
            vals['created_menu_id'] = self.create_menu(
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for r in self.browse(cr, uid, ids, context=context):
            if r.created_menu_id:
                self.delete_menu(
                    cr, uid, r.created_menu_id.id, context=context)

            values_obj.unlink(cr, uid, values_obj.search(
                cr, uid, [('value', '=', "ir.actions.report.xml,%s" % r.id)]),
                context=context)
    ...
    def update_pentaho(self, cr, uid, ids, context=None):
        ...
        for report in self.browse(cr, uid, ids):
            values_ids = values_obj.search(
                cr, uid, [('value', '=', "ir.actions.report.xml,%s"
                % report.id)])

            if report.pentaho_filename:
                if report.pentaho_load_file:
                    # if we receive a filename and no content, this has
                    # probably been loaded by a process other than the
                    # standard client, such as a data import
                    # in this case, we expect the filename to be a fully
                    # specified file within a module from which we
                    # load the file data
                    super(report_xml, self).write(
                        cr, uid, [report.id], {
                            'pentaho_filename': os.path.basename(report.pentaho_filename),
                ...
                # we are no longer relying on report_rml to contain a name
                # at all - for clarity, though, still store it...
                super(report_xml, self).write(
                    cr, uid, [report.id],
                    {"report_rml": report.pentaho_filename})

                if not report.linked_menu_id and \
                report.pentaho_filename.endswith(".prpt"):
                    data = {
                        'name': report.name,
                        'model': report.model,
                        'key': 'action',
                    ...
                    if not values_ids:
                        ...
                        values_obj.write(
                            cr, uid, values_ids, data, context=context)
                    values_ids = []
                ...
                super(report_xml, self).write(
                    cr, uid, [report.id], {"pentaho_file": False})
    ...
    def read_content_from_file(self, name):
        ...
        if not path_found:
            raise orm.except_orm(
                _('Error'), _('Could not locate path for file %s') % name)
    ...
    def pentaho_validate_params(self, cr, uid, report,
                                param_vals, context=None):
        """Validate a list of passed parameters against the defined params for
        a Pentaho report.

        Raises an exception if any of the params are invalid.

        @param report: Browse object on the ir.actions.report.xml
                       record for the report.
        @param param_vals: Dict with parameter values to
                           pass to the report. These are python
            data types prior to conversion for passing to the Pentaho server.
        """
        param_defs = core.fetch_report_parameters(
            cr, uid, report.report_name, context=context)
        ...
        for pdef in param_defs:
            ...
            if pname in val_names:
                ...
                if pdef.get('default_value', False):
                    ...
                    if pdef.get('is_mandatory', False):
                        raise orm.except_orm(
                            _('Error'),
                            _("Report '%s'. No value passed for mandatory "
                            "report parameter '%s'.")
                            % (report.report_name, pname))
                    continue
            ...
            if not value_type in JAVA_MAPPING:
                raise orm.except_orm(
                    _('Error'),
                    _("Report '%s', parameter '%s'. Type '%s' not supported.")
                    % (report.report_name, pname, pdef.get('value_type', '')))
            ...
            if not local_type in PARAM_VALUES:
                raise orm.except_orm(
                    _('Error'),
                    _("Report '%s', parameter '%s'. Local type '%s' not"
                    " supported.") % (report.report_name, pname, local_type))
            if not isinstance(param_val, PARAM_VALUES[local_type]['py_types']):
                raise orm.except_orm(_(
                    'Error'), _("Report '%s', parameter '%s'. Passed value is"
                    " '%s' but must be one of '%s'.")
                    % (report.report_name, pname, param_val.__class__.__name__,
                        PARAM_VALUES[local_type]['py_types']))
            ...
            if converter:
                try:
                    ...
                    raise orm.except_orm(
                        _('Error'),
                        _("Report '%s', parameter '%s'. Passed value "
                        "'%s' failed data conversion to type '%s'.\n%s")
                        % (report.report_name,
                            pname, param_val, local_type, str(e)))
        ...
        if val_names:
            raise orm.except_orm(
                _('Error'),
                _("Report '%s'. Parameter values not"
                " required by report: %s") % (report.report_name, val_names))

    def pentaho_report_action(self, cr, uid, service_name, active_ids=None,
                            param_values=None, context=None):
        ...
        @param service_name: The report service
                             name (without leading 'report.').
        ...
        report_ids = self.search(
            cr, uid, [('report_name', '=', service_name)], context=context)
        if report_ids:
            report = self.browse(cr, uid, report_ids[0], context=context)
        if (not report) or (not report.is_pentaho_report):
            raise orm.except_orm(
                _('Error'),
                _("Report '%s' is not a Pentaho report.") % service_name)

        if (not active_ids) and (not param_values):
            raise orm.except_orm(
                _('Error'),
                _("Report '%s' must be passed active ids"
                " or parameter values.") % service_name)

        datas = {'model': report.model, 'output_type': report.report_type}

        if active_ids:
            datas['ids'] = active_ids

        if param_values:
            self.pentaho_validate_params(
                cr, uid, report, param_values, context=context)
