...
"""
Define mappings as functions, which can be passed the data format to
...
JAVA_MAPPING = {
    ...
    'java.util.Date': lambda x:
        TYPE_DATE if x and not('H' in x) else TYPE_TIME,
...
PARAM_VALUES = {
    TYPE_STRING: {
        ...
        'if_false': '', 'py_types': (str, unicode)},
    TYPE_BOOLEAN: {
        'value': PARAM_XXX_BOOLEAN_VALUE,
        'if_false': False, 'py_types': (bool)},
    TYPE_INTEGER: {
        'value': PARAM_XXX_INTEGER_VALUE,
        'if_false': 0, 'py_types': (int, long)},
    TYPE_NUMBER: {
        'value': PARAM_XXX_NUMBER_VALUE,
        'if_false': 0.0, 'py_types': (float,),
        'convert': lambda x: float(x)},
    TYPE_DATE: {
        'value': PARAM_XXX_DATE_VALUE,
        'if_false': '', 'py_types': (str, unicode),
        'convert': lambda x: datetime.strptime(x, '%Y-%m-%d'),
        'conv_default': lambda x: datetime.strptime(
            x.value, '%Y%m%dT%H:%M:%S').strftime('%Y-%m-%d')},
    TYPE_TIME: {
        ...
        'conv_default': lambda x: datetime.strptime(
            x.value, '%Y%m%dT%H:%M:%S').strftime('%Y-%m-%d %H:%M:%S')},
}
...
RESERVED_PARAMS = {
    ...
    'user_name': lambda s: s.pool.get('res.users').browse(
        s.cr, s.uid, s.uid, context=s.context).name,
}
