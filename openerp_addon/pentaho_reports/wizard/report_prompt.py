...
class report_prompt_class(orm.TransientModel):
    ...
    def _parse_one_report_parameter_default_formula(self, formula, type,
                                                    context=None):
        """reviously, we were not getting a default value if the report had
        a default formula, so we endeavoured to generate a value.

        However, default formulas are now (correctly) being evaluated
        ...
        if formula == '=NOW()':
            ...
            if context and context.get('tz'):
                now = pytz.timezone('UTC').localize(
                    now, is_dst=False).astimezone(pytz.timezone(context['tz']))
    ...
    def _parse_one_report_parameter(self, parameter, context=None):
        ...
        Optional:
            ...
            'multi_select' : True if list of values allowed.
                             However, we can not prompt for this, so it
        ...
        if not value_type in JAVA_MAPPING:
            raise orm.except_orm(
                _('Error'),
                _("Unhandled parameter type (%s).")
                % parameter.get('value_type', ''))

        if not parameter.get('name', False):
            raise orm.except_orm(
                _('Error'), _("Unnamed parameter encountered."))
        ...
        if parameter['name'] in context.get('pentaho_defaults', {}).keys():
            ...
            # default date or datetime is passed from Pentaho in
            # local time without a timezone. If it is a datetime value,
            # we need to convert to UTC for OpenERP to handle it correctly.
        ...
        for parameter in report_parameters:
            if not parameter.get('name') in RESERVED_PARAMS.keys():
                if not parameter.get('attributes',{}):
                    raise orm.except_orm(
                        _('Error'),
                        _("Parameter received with no attributes."))

                result.append(self._parse_one_report_parameter(
                    parameter, context=context))

        if len(result) > MAX_PARAMS + 1:
            raise orm.except_orm(
                _('Error'),
                _("Too many report parameters (%d).")
    ...
    def _setup_parameters(self, cr, uid, context=None):
        ...
        report_ids = ir_actions_obj.search(
            cr, uid, [('report_name', '=', context.get('service_name', ''))],
            context=context)
        if not report_ids:
            raise orm.except_orm(
                _('Error'), _("Invalid report associated with menu item."))

        report_record = ir_actions_obj.browse(
        ...
        if not self.paramfile or self.paramfile['report_id'] != report_ids[0] or self.paramfile['prpt_content'] != prpt_content or self.paramfile['context'] != context:
            ...
            self.parameters = self._parse_report_parameters(
                report_parameters, context=context)
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        result = super(report_prompt_class, self).default_get(
            cr, uid, fields, context=context)

        result.update({
            'report_name': self.pool.get('ir.actions.report.xml').browse(
                cr, uid, self.paramfile['report_id'], context=context).name,
            'output_type': self.pool.get('ir.actions.report.xml').browse(
                cr, uid, self.paramfile['report_id'],
                context=context).pentaho_report_output_type or
                DEFAULT_OUTPUT_TYPE,
        })
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
        ...
        # fields_view_get() is called during module installation,
        # in which case there is no service_name in the context.
        if context.get('service_name', '').strip() == '':
            return super(report_prompt_class, self).fields_view_get(
                cr, uid, view_id=view_id, view_type=view_type,
                context=context, toolbar=toolbar, submenu=submenu)

        def add_field(result, field_name,
                    selection_options=False, required=False):
        ...
        result = super(report_prompt_class, self).fields_view_get(
            cr, uid, view_id=view_id, view_type=view_type,
            context=context, toolbar=toolbar, submenu=submenu)
        ...
        for index in range(0, len(self.parameters)):
            add_field(result,
                ...
                selection_options=self.parameters[index].get(
